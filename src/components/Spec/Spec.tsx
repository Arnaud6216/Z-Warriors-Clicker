...
function Spec() {
    ...
    const { count, setCount, concentrationCount, setConcentrationCount, concentrationCost, setConcentrationCost, concentrationIncrement, gif, setGif, gifSrc } = context;
    const [style, setStyle] = useState("spec-option");

    // Changer le style du bouton de concentration en fonction du coût
    useEffect(() => {
        setStyle(count >= concentrationCost ? "spec-option-available" : "spec-option");
    ...
    const handleClickSsj = () => {
        if (count >= 50 && gif !== 1) { // Vérifie si le gif n'est pas déjà en état 1
